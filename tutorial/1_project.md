...
- **Operating System**: Select the operating system. If there are no available options, select 'Input Manually' and enter it.
- **Security Mail (Vulnerability)**: Indicate whether to receive Vulnerability emails for the project.
...
- **Distribution Type**: Select one of the following distribution types.
    - **General Model**: Distributing general model, software, or applications.
    - **Transfer in-house**: Transferring to other organization within the company transfers.
    - **B2B**: Providing to another company.
    - **Preceding Software / In-house only**: Preliminary development or not intended for external distribution.
...
- **OSS Notice**: Select the appropriate OSS notice format.
    - **General**: FA standard OSS notice.
    - **Platform-generated**: The OSS notice is generated by a platform (e.g., Android, Yocto) and embedded in the product.
    - **N/A**: No OSS notice is required.
- **Priority**: Select the priority level for the OSS notice issuance request.  
    ...
    - Frequent selection of P0 may lead to the organization being subject to OSC process assessments.
...
- **Watcher**: Add users who want to share the project's information and status.  
    - Users added as a Watcher have the same viewing and editing permissions as the Creator, and receives project-related emails from the system.  
...
- Click the **Request** button on the **BOM** tab after filling out the **3rd Party, DEP, SRC,** and **BIN** tabs for a review request.  
    ...
    - **DEP**: Enter the dependency analysis results.  
    - **SRC**: Enter the source code analysis results.  
    - **BIN**: Enter the binary analysis results.  
...
- After filling out the **BIN(Android/Yocto)** tab, click the **Request** button to request a review.  
...
You can choose from the following methods to import 3rd Party Software information.  
...
- **How to Automatically Fill Using a Scanner**
    - A report can be automatically generated with OSS information filled in using the FOSSLight Dependency Scanner.

#### 2-3. SRC Tab
![prj](images/4_project_iden_srctap.PNG)
**SRC Tab Writing Process**  
- If OSS is included in the source code: <U>Document OSS information for each source code → Save</U>  
...
- **Manually Enter in the OSS Table**  
    - Click the "+" button in the top left corner of the OSS Table to input the OSS information.
- **How to Bulk Load OSS Information**  
    1. Upload the FOSSLight Report containing the OSS List in the "Upload Analysis Result" section.  
        - You can download the compatible FOSSLight Report format by clicking the "Export" button in the top right corner.  
...
Collect the OSS lists written in the 3rd Party, DEP, SRC, and BIN tabs and request a review.
![prj](images/4_project_iden_bomtap.png)

##### How to Request a Review
1. Click the **Save** button.  
   - This will consolidate the OSS lists from the 3rd Party, DEP, SRC, and BIN tabs.  
2. Review the issues highlighted in the [Warning messages](#warning).  
...
- The OSS notice is automatically generated when the Packaging stage is confirmed. If any modifications are needed, they can be made in the Notice tab.
